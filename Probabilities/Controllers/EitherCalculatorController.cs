...
{
    //Separate API endpoint for using the either calculation.
    [Route("api/either")]
    [ApiController]
    public class EitherCalculatorController : ControllerBase
    {
        //This makes use of the log service so when a valid calculation is made it is stored in the JSON file.
        public EitherCalculatorController(JsonFileLogService logService)
        {
        ...
        public IActionResult Get([Required]decimal inputOne, [Required]decimal inputTwo)
        {
            ...
            {
                //Perform the given calculation for this formula P(A)+P(B) - P(A)P(B).
                decimal Result = inputOne + inputTwo - (inputOne * inputTwo);

                //Create a new log object, with current datetime stamp, the either function as calc type and the inputs and result.
                Log NewLog = new Log();
                NewLog.Date = DateTime.Now;
                NewLog.CalculationType = "Either";
